...
const ESTRELLAS: ReadonlyArray<readonly [number, number, number, number]> = [
  ...
  // ── Franja baja, hasta pasando la línea del horizonte (~548) ──────────
  // Se dibujan DEBAJO del agua en el DOM, así que con la escena en su sitio
  // quedan tapadas; se ven en el tramo que corre detrás del hero, donde el
  // mar está hundido y sin ellas quedaba un vacío. Van apagándose hacia
  // abajo: cerca del horizonte el cielo siempre lava las estrellas.
  [72, 496, 1.1, 0.055], [216, 520, 1.0, 0.045], [352, 486, 1.2, 0.06],
  [512, 528, 1.0, 0.04], [648, 500, 1.1, 0.05], [792, 536, 1.0, 0.04],
  [936, 494, 1.2, 0.055], [1088, 524, 1.0, 0.045], [1224, 492, 1.1, 0.05],
  [1372, 530, 1.0, 0.04],
  [140, 566, 1.0, 0.038], [300, 596, 0.9, 0.03], [468, 572, 1.0, 0.035],
  [612, 604, 0.9, 0.028], [760, 578, 1.0, 0.034], [904, 610, 0.9, 0.026],
  [1052, 574, 1.0, 0.033], [1200, 602, 0.9, 0.027], [1336, 570, 1.0, 0.032],
  [196, 646, 0.9, 0.024], [420, 668, 0.8, 0.02], [688, 644, 0.9, 0.023],
  [860, 672, 0.8, 0.019], [1128, 650, 0.9, 0.022], [1300, 676, 0.8, 0.018],
] as const;
...
export function FaroEscena() {
  ...
  return (
    ...
    >
      ...
      {/* Horizonte, mar y faro van en wrappers propios: la cámara escribe
          transforms en [data-capa], así que cualquier desplazamiento extra
          necesita su propia capa o se lo pisa en el frame siguiente. Los usa
          el tramo que corre detrás del hero para hundir TODO el mundo bajo el
          borde. Van las CUATRO capas: mover solo el faro dejaba la línea del
          horizonte quieta y la escena se partía al medio, y dejar el cielo
      ...
      {/* Muelle y primer plano RETIRADOS por decisión del cliente
          (2026-09): el faro queda solo sobre su islote. La diagonal oscura
          del muelle se comía el tercio inferior y cruzaba justo por donde
          pasan los verbos del método. Las capas y sus componentes siguen en
          rama gaston/hero-faro por si se quieren recuperar (ahí están
          CapaMuelle y CapaForeground completas). OJO: las escalas del método
          (S3) estaban pensadas "a lo largo del muelle"; sin él quedan sobre
          el agua. */}
    </div>
  );
}
