...
export function Boton({ children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      {...props}
      className="w-full rounded-lg bg-naranja-accion px-6 py-3 font-medium text-azul-principal transition-colors hover:bg-naranja-accion/90 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-azul-principal disabled:cursor-not-allowed disabled:opacity-60"
    >
      {children}
    </button>
  );
